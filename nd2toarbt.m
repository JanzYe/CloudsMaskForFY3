function [im_th,TOAref,ijdim_ref,ul,zen,azi,zc,B1Satu,B2Satu,B3Satu,resolu]=nd2toarbt(path)
...
im_th=single(n_B(:,:,5));
% Band6
im_B5=single(n_B(:,:,6)); %与landsat比较，5,6波段要互换
...
im_B1(im_B1 < 0) = Lmax(1);
im_B2(im_B2 < 0) = Lmax(2);
im_B3(im_B3 < 0) = Lmax(3);
im_B4(im_B4 < 0) = Lmax(4);
im_B5(im_B5 < 0) = Lmax(5);
im_th(im_th < 0) = Lmax(6);
im_B7(im_B7 < 0) = Lmax(7);

% im_B1 = single(gscale(im_B1));
% im_B2 = single(gscale(im_B2));
% im_B3 = single(gscale(im_B3));
% im_B4 = single(gscale(im_B4));
% im_B5 = single(gscale(im_B5));
% im_th = single(gscale(im_th));
% im_B7 = single(gscale(im_B7));

% % %对比度拉伸
im_B1 = intrans(im_B1,'stretch').*255;
im_B2 = intrans(im_B2,'stretch').*255;
im_B3 = intrans(im_B3,'stretch').*255;
im_B4 = intrans(im_B4,'stretch').*255;
im_B5 = intrans(im_B5,'stretch').*255;
im_th = intrans(im_th,'stretch').*255;
im_B7 = intrans(im_B7,'stretch').*255;


% 
% im_B1 = intrans(im_B1,'gamma',1).*255;
% im_B2 = intrans(im_B2,'gamma',1).*255;
% im_B3 = intrans(im_B3,'gamma',1).*255;
% im_B4 = intrans(im_B4,'gamma',1).*255;
% im_B5 = intrans(im_B5,'gamma',1).*255;
% im_th = intrans(im_th,'gamma',1).*255;
% im_B7 = intrans(im_B7,'gamma',1).*255;
...
% ND to radiance first   辐射校正后再做云识别的  而我要在辐射校正前做
...
esun=[2007.95, 1741.58, 1558.31, 957.31, 233.65, -1.0, 97.33];
...
% 
% figure(212);
% imshow(im_B1,[min(min(im_B1)),max(max(im_B1))])
...
K1_L7 =  1.438833*875.1379;
K2_L7 = 1.191066*power(10,-5)*power(875.1379,3);
